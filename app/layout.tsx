...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  // Verificamos la sesión cada vez que cambia la ruta
  useEffect(() => {
      ...
      <body className="bg-slate-950 text-slate-200 min-h-screen">
        <nav className="bg-slate-950/80 backdrop-blur-md border-b border-slate-800 p-4 sticky top-0 z-50">
          <div className="max-w-7xl mx-auto flex justify-between items-center">
            ...
            {/* Opciones de navegación: SOLO si está logueado */}
            {isLoggedIn && (
              <div className="flex items-center gap-6 animate-in fade-in slide-in-from-right-4">
                <Link href="/asistencia" className="text-sm font-medium hover:text-sky-400 transition-colors">Asistencia</Link>
                <Link href="/comunicaciones" className="text-sm font-medium hover:text-sky-400 transition-colors">Mensajería</Link>
                <button 
                  onClick={() => {
                    localStorage.removeItem('session');
                    window.location.href = '/'; // Redirige al inicio para limpiar estado
                  }}
